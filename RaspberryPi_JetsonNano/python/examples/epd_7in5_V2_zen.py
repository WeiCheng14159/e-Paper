...
class Display:
    ...
    def __init__(self, epd):
        self.image = Image.new(
            '1', (self.DISPLAY_WIDTH, self.DISPLAY_HEIGHT), 255)
...
class ImageTask(Task):
    def __init__(self, display, image_dir, update_interval=5):
        super().__init__(display, (0, 30, display.DISPLAY_WIDTH,
                                   display.DISPLAY_HEIGHT), update_interval)
    ...
    def find_image_files(self, root_dir):
        ...
        if not os.path.exists(root_dir):
            return []
        file_list = [os.path.join(subdir, file) for subdir, dirs, files in os.walk(
            root_dir) for file in files if file.lower().endswith(extensions)]
        file_list.sort()
        return file_list
...
def main():
    ...
    zen_img_dir = os.path.join(os.path.dirname(
        os.path.dirname(os.path.realpath(__file__))), 'zen_img')
